...
def estruturas(json_path, dxf_path):
    ...
    for item in ambientes:
        ...
        for fnd in item.get("fundacoes", []):
            ...
            if is_viga:
                ...
                df_ref.loc[idx, ("Forma em Madeira", "A [m²]")]= fnd.get("areaForma", "")
            
            else:
                df_ref.loc[idx, ("Valores por Peça", "L [m]")]= ""
                df_ref.loc[idx, ("Valores por Peça", "h [m]")]= fnd.get("profundidade", "")
                df_ref.loc[idx, ("Valores por Peça", "C [m]")]= ""
                df_ref.loc[idx, ("Valores Totais", "Lastro [m³]")]= fnd.get("volumeLastro", "")
                df_ref.loc[idx, ("Valores Totais", "Concreto [m³]")]= fnd.get("volumeConcreto", "")
                df_ref.loc[idx, ("Valores Totais", "Ferragem [KgF]")]= fnd.get("pesoFerragem", "")
                df_ref.loc[idx, ("Valores Totais", "Estribo [KgF]")]= fnd.get("pesoEstribo", "")
                df_ref.loc[idx, ("Forma em Madeira", "L [m]")]= ""
                df_ref.loc[idx, ("Forma em Madeira", "C [m]")]= ""
                df_ref.loc[idx, ("Forma em Madeira", "A Tot [m²]")]= fnd.get("areaForma", "")
        ...
        for sup in item.get("superestrutura", []):
            ...
            df_ref.loc[idx, ("Forma em Madeira", "Janela lança/o [m]")]= sup.get("janelaLancamento", "")
        ...
        for mad in item.get("madeira", []):
            ...
            df_madeira.loc[idx, ("Telhamento", "T.Tipo")]= mad.get("tipoTelhamento", "")
            ...
            if _ma and _txt.get("posicao"):
                ...
                _aidx = next(
                    (i for i, p in enumerate(_parts)
                     if re.search(r"\d+[.,]\d+\s*m²", p, re.IGNORECASE)), -1
                )
                _raw = " ".join(_parts[:_aidx]) if _aidx > 0 else _parts[0]
                _nome = re.sub(r"\\[^;\\]+;", "", _raw).replace("{","").replace("}","")
    ...
    def _get_amb(cx, cy):
        ...
        best = min(ambientes_cad, key=lambda a: math.hypot(cx - a["pos"][0], cy - a["pos"][1]))
    ...
    _pilar_lines = [
        ...
        and e.get("tipo") == "LINE"
        and "dados" in e
        and "inicio" in e["dados"] and "fim" in e["dados"]
    ]
    _pilar_circs = [
        e for e in entidades
        if e.get("layer") == "Estrutural - Pilares"
    ...
    for _i in range(len(_pilar_lines)):
        ...
        while _q:
            ...
            _comp.append(_cur)
            if _cur < len(_edges):
                for _nb in _adj.get(_edges[_cur][1], []) + _adj.get(_edges[_cur][2], []):
                    if _nb not in _visited:
                        _visited.add(_nb); _q.append(_nb)
        _comps.append(_comp)
    ...
    for _comp in _comps:
        ...
        _pts_x = [c for i in _comp for c in [_pilar_lines[i]["dados"]["inicio"][0], _pilar_lines[i]["dados"]["fim"][0]]]
        _pts_y = [c for i in _comp for c in [_pilar_lines[i]["dados"]["inicio"][1], _pilar_lines[i]["dados"]["fim"][1]]]
    ...
    _meta_v = [
        {"idx": i, "linha": v, "ang": _ang(v), "med": _med(v), "comp": v["dados"].get("comprimento", 0)}
    ...
    for _grp in _grupos_v:
        _pts_x = [c for m in _grp for c in [m["linha"]["dados"]["inicio"][0], m["linha"]["dados"]["fim"][0]]]
        _pts_y = [c for m in _grp for c in [m["linha"]["dados"]["inicio"][1], m["linha"]["dados"]["fim"][1]]]
        _cx = sum(_pts_x) / len(_pts_x)
        _cy = sum(_pts_y) / len(_pts_y)
        _amb = _get_amb(_cx, _cy)
    ...
    for _i in range(len(_laje_lines)):
        ...
        while _q:
            ...
            if _cur < len(_edges_lj):
                for _nb in _adj_lj.get(_edges_lj[_cur][1], []) + _adj_lj.get(_edges_lj[_cur][2], []):
                    if _nb not in _visited_lj:
                        _visited_lj.add(_nb); _q.append(_nb)
    ...
    for _comp in _comps_lj:
        ...
        _pts_x = [c for i in _comp for c in [_laje_lines[i]["dados"]["inicio"][0], _laje_lines[i]["dados"]["fim"][0]]]
        _pts_y = [c for i in _comp for c in [_laje_lines[i]["dados"]["inicio"][1], _laje_lines[i]["dados"]["fim"][1]]]
        _cx = sum(_pts_x) / len(_pts_x)
        _cy = sum(_pts_y) / len(_pts_y)
        _amb = _get_amb(_cx, _cy)
    ...
    _bloco_lines = [
        e for e in entidades 
        if e.get("layer") and "alvenaria" in str(e.get("layer")).lower() 
    ...
    for _g in _grupos_bl:
        _px = [c for m in _g for c in [m["linha"]["dados"]["inicio"][0], m["linha"]["dados"]["fim"][0]]]
        _py = [c for m in _g for c in [m["linha"]["dados"]["inicio"][1], m["linha"]["dados"]["fim"][1]]]
        ...
        _L = round(_dist_pt_reta(_lng[1]["med"][0], _lng[1]["med"][1], _lng[0]["linha"]), 2) if len(_lng) >= 2 else 0.15
        ...
        for col in [("", "h [m]"), ("", "Concreto [m³]"), ("", "Ferragem [KgF]"), ("", "Estribo [KgF]"), ("Forma em Madeira", "A [m²]")]:
 ...
 # ── 3.2.9 Bermas ───────────────────────────────────────────────
    layers_bermas = ["ARQ - Desnível Terreno", "ARQ - Desnível", "ARQ - Guias Calçadas Sarjetas"]
    bermas_raw = []
 ...
 # ── 3.3.1 Pilares metálicos ───────────────────────────────────────────────
            ...
            for t in dados_automaticos.get("textos", []):
                ...
                if math.hypot(pos[0] - t_pos[0], pos[1] - t_pos[1]) < 130:
                    ...
                    if not any(tp in cont for tp in ["m²", "M2", "AREA"]):
                        p_limpo = re.sub(r"\\[^;\\]+;", "", t.get("conteudo", "")).strip()
                        perfil = p_limpo.replace("{", "").replace("}", "")
                        break
            
            pilares_met_data.append({"amb": amb, "l": dim_l, "b": dim_b, "perfil": perfil})
    ...
    df_pilares = agrupar_elementos(df_pilares,   ("", "Ambiente"), ("", "L [m]"), ("", "C [m]"))
    df_vigas = agrupar_elementos(df_vigas,     ("", "Ambiente"), ("", "L [m]"), ("", "C [m]"))
    df_lajes = agrupar_elementos(df_lajes,     ("", "Ambiente"), ("", "L [m]"), ("", "C [m]"))
    df_contencao = agrupar_elementos(df_contencao, ("", "Ambiente"), ("", "L [m]"), ("", "C [m]"))
    df_bloco = agrupar_elementos(df_bloco, ("", "Ambiente"), ("", "L [m]"), ("", "C [m]"))
    ...
    ordem_secoes = [
        "3.1 Fundações", "3.1.1 Sistema Viga Baldrame", "3.1.2 Estacas e Blocos de Coroamento", "3.1.3 Estacas", "3.1.4 Sapatas Isoladas", "3.1.5 Radier",
        "3.1.6 Sistema Sapata Corrida", "3.2 Superestrutura em Concreto", "3.2.1 Pilares", "3.2.2 Vigas", "3.2.3 Lajes", "3.2.4 Paredes de Contenção",
        "3.2.5 Bloco Estrutural", "3.2.6 Placa", "3.2.7 Passarelas", "3.2.8 Escadas", "3.2.9 Pórticos", "3.2.10 Bermas", "3.2.11 Recuperação de Estruturas em Concreto","3.3 Estruturas Metálicas", 
        "3.3.1 Pilares", "3.3.2 Vigas", "3.3.3 Lajes", "3.3.4 Passarelas", "3.3.5 Escadas", "3.3.6 Pórticos", "3.3.7 Para-Balas", "3.3.8 Recuperação de Estruturas Metálicas","3.4 Estruturas em Madeira",
    ]
 
    tabela_map = {
        "3.1.1 Sistema Viga Baldrame":(df_viga_baldrame,   colunas_estruturas_viga, "viga"),
        "3.1.2 Estacas e Blocos de Coroamento":(df_estacas_blocos, colunas_estruturas_fundacoes, "fundacoes"),
        "3.1.3 Estacas":(df_estacas, colunas_estruturas_fundacoes, "fundacoes"),
        "3.1.4 Sapatas Isoladas":(df_sapatas, colunas_estruturas_fundacoes, "fundacoes"),
        "3.1.5 Radier":(df_radier, colunas_estruturas_fundacoes, "fundacoes"),
        "3.1.6 Sistema Sapata Corrida":(df_sapata_corrida, colunas_estruturas_fundacoes, "fundacoes"),
        "3.2.1 Pilares":(df_pilares, colunas_estruturas_concreto, "concreto"),
        "3.2.2 Vigas":(df_vigas, colunas_estruturas_concreto, "concreto"),
        "3.2.3 Lajes":(df_lajes, colunas_estruturas_concreto, "concreto"),
        "3.2.4 Paredes de Contenção":(df_contencao, colunas_estruturas_concreto, "concreto"),
        "3.2.5 Bloco Estrutural":(df_bloco, colunas_estruturas_concreto, "concreto"),
        "3.2.6 Placa":(df_placa, colunas_estruturas_concreto, "concreto"),
        "3.2.7 Passarelas":(df_passarelas, colunas_estruturas_concreto, "concreto"),
        "3.2.8 Escadas": (df_escadas, colunas_estruturas_escada, ""),
        "3.2.9 Pórticos":(df_porticos, colunas_estruturas_concreto, "concreto"),
        "3.2.10 Bermas":(df_bermas, colunas_estruturas_concreto,  "concreto"),
        "3.2.11 Recuperação de Estruturas em Concreto": (df_recuperacao, colunas_estruturas_concreto, "concreto"),
        "3.3.1 Pilares":(df_pilares_met, colunas_estruturas_metalica, "metalica"),
        "3.3.2 Vigas":(df_vigas_met, colunas_estruturas_metalica, "metalica"),
        "3.3.3 Lajes": (df_lajes_met, colunas_estruturas_metalica, "metalica"),
        "3.3.4 Passarelas":(df_passarelas_met, colunas_estruturas_metalica, "metalica"),
        "3.3.5 Escadas":(df_escadas_met, colunas_estruturas_metalica, "metalica"),
        "3.3.6 Pórticos":(df_porticos_met, colunas_estruturas_metalica, "metalica"),
        "3.3.7 Para-Balas":(df_para_balas, colunas_estruturas_metalica, "metalica"),
        "3.3.8 Recuperação de Estruturas Metálicas": (df_recuperacao_met, colunas_estruturas_metalica, "metalica"),
        "3.4 Estruturas em Madeira":(df_madeira, colunas_estruturas_madeira, "madeira"),
    }
