...
def telefonia(dados_manuais, dados_automaticos):
    ...
    def _is_calha_rede(cx, cy, limite=3000):
        ...
        _mais_prox = min(
            _pos_calhas_rede,
            key=lambda c: math.hypot(cx - c["pos"][0], cy - c["pos"][1]),
        )
    ...
    for e in _ents_all:
        ...
        if any(
            k in _layer
            for k in ["LOGICA", "LÓGICA", "DADOS", "TELEFONIA", "TV", "CFTV", "REDE"]
        ):
    ...
    for amb in sorted(all_ambs):
        ...
        for cab in cabeamentos:
            ...
            df_telefonia.loc[idx, ("Quadros de Rede", "Circuito", "")] = cab.get(
                "circuito", ""
            )
            df_telefonia.loc[idx, ("Quadros de Rede", "Tipo", "")] = ""
            df_telefonia.loc[idx, ("Quadros de Rede", "Qtd", "")] = item.get(
                "quadrosRede", ""
            )

            man_rede = (
                cab.get("comprimento", "")
                if cab.get("circuito", "") in ("dados", "rede")
                else ""
            )
            auto_rede = (
                round(calha_auto + tubo_auto, 2) if (calha_auto + tubo_auto) > 0 else ""
            )
            df_telefonia.loc[idx, ("Cabeamentos", "Rede", "C [m]")] = (
                auto_rede if auto_rede != "" else man_rede
            )

            df_telefonia.loc[idx, ("Cabeamentos", "Patch Cord", "C [m]")] = item.get(
                "patchCords", ""
            )

            man_cams = item.get("cameras", "")
            auto_cams = cams_auto * 15 if cams_auto > 0 else ""
            df_telefonia.loc[idx, ("Cabeamentos", "Câmera", "C [m]")] = (
                auto_cams if auto_cams != "" else man_cams
            )

            df_telefonia.loc[idx, ("Dutos", "Eletrocalha", "L [cm]")] = (
                10 if calha_auto > 0 else ""
            )
            df_telefonia.loc[idx, ("Dutos", "Eletrocalha", "h [cm]")] = (
                5 if calha_auto > 0 else ""
            )
            df_telefonia.loc[idx, ("Dutos", "Eletrocalha", "C [m]")] = (
                round(calha_auto, 2) if calha_auto > 0 else ""
            )

            df_telefonia.loc[idx, ("Dutos", "Septo", "Elétrica [cm]")] = ""
            df_telefonia.loc[idx, ("Dutos", "Septo", "Lógica [cm]")] = ""

            df_telefonia.loc[idx, ("Dutos", "Eletroduto", "DN [mm]")] = (
                25 if tubo_auto > 0 else ""
            )

            man_tubo = cab.get("comprimento", "")
            auto_tubo = round(tubo_auto, 2) if tubo_auto > 0 else ""
            df_telefonia.loc[idx, ("Dutos", "Eletroduto", "C [m]")] = (
                auto_tubo if auto_tubo != "" else man_tubo
            )
            ...
            df_telefonia.loc[idx, ("Aterramento", "Haste", "Qtd")] = item.get(
                "hastesAterramento", ""
            )
            df_telefonia.loc[idx, ("Aterramento", "Caixa Inspeção", "Qtd")] = item.get(
                "caixasInspecao", ""
            )
