...
def eletricas(json_path, dxf_path):
    ...
    def _get_amb_ele(cx, cy):
        if not _ambientes_ele:
            return "GERAL"
        return min(_ambientes_ele, key=lambda a: math.hypot(cx - a["pos"][0], cy - a["pos"][1]))["nome"]

    _circuitos_cad = []
    for _txt in _txts_all:
        _layer = _txt.get("layer", "").upper()
        if "ELE" in _layer or "ELÉTRICA" in _layer:
            _c_limpo = limpar_texto_cad(_txt.get("conteudo", "")).replace("(", "").replace(")", "").strip()
            if re.match(r"^C\d+$", _c_limpo, re.IGNORECASE):
                _circuitos_cad.append({"nome": _c_limpo.upper(), "pos": _txt.get("posicao", [0, 0, 0])})

    def _get_circuito_ele(cx, cy):
        if not _circuitos_cad:
            return "C?"
        return min(_circuitos_cad, key=lambda c: math.hypot(cx - c["pos"][0], cy - c["pos"][1]))["nome"]
    ...
    for _txt in _txts_all:
        ...
        if "ELE" in _layer or "ELÉTRICA" in _layer:
            ...
            _m_cabo1 = re.search(r"(\d+)?\s*#\s*(\d+[.,]\d+|\d+)\s*(?:mm)?", _c_limpo, re.IGNORECASE)
            _m_cabo2 = re.search(r"\b(\d+[.,]\d+|\d+)\s*mm[²2]?\b", _c_limpo, re.IGNORECASE)
    ...
    _padrao_tubo = re.compile(r"(?:%%[cC]|ø|Ø|DN)\s*(\d+[,.]?\d*(?:/\d+)?)\s*(\"|mm)?", re.IGNORECASE)
    _mapa_diametros = {"1/4": "25", "12": "20", "5/8": "20", "3/4": "25", "1": "32", "1.1/4": "40", "1.1/2": "50", "2": "60"}
    ...
    for _txt in _txts_all:
        ...
        if "ELE" in _layer or "ELÉTRICA" in _layer:
            ...
            if _m_tubo:
                ...
                _textos_tubos.append({"dn": _dn, "material": _mat, "pos": _txt.get("posicao", [0, 0, 0])})

    def _get_tubo_ele(cx, cy):
        if not _textos_tubos: return ("25", "PVC Liso")
        _best = min(_textos_tubos, key=lambda t: math.hypot(cx - t["pos"][0], cy - t["pos"][1]))
        ...
        if _layer in ["ELE-CIRCUITO", "ELÉTRICA"]:
            ...
            if _tipo in ["LINE", "LWPOLYLINE", "POLYLINE"]:
                ...
                if _comp > 0:
                    ...
                    if not _pts and "inicio" in _e.get("dados", {}): _pts = [_e["dados"]["inicio"]]
                    if _pts:
                        _cx, _cy = _pts[0][:2]
                        _amb = _get_amb_ele(_cx, _cy)
                        _circ = _get_circuito_ele(_cx, _cy)
    ...
    for _txt in _txts_all:
        ...
        if "ELE" in _layer or "ELÉTRICA" in _layer:
            ...
            if re.match(r"^(QDG|QDC|QD)\b", _c_limpo, re.IGNORECASE) or "QUADRO" in _c_limpo.upper():
                if len(_c_limpo) < 50:
                    _dados_df2[_amb][_circ][("Quadros de energia", "Tipo", "")] = _c_limpo.upper()

            _m_cond = re.search(r'CONEXÃO TIPO\s*["\']?([A-Z]+)["\']?', _c_limpo, re.IGNORECASE)
            if _m_cond:
    ...
    for _amb, _qtd in _caixas_aterramento.items():
        _dados_df2[_amb]["GERAL"][("Aterramento", "Caixa Inspeção", "Quantidade")] = _qtd
    ...
    tabela_map = {
        "Circuitos Aéreos": (df, colunas_eletricas, "eletrico"),
        "Circuitos Subterrâneos": (df1, colunas_eletricas1, "eletrico"),
        "Circuitos Internos": (df2, colunas_eletricas2, "eletrico"),
    }
    return tabela_map
