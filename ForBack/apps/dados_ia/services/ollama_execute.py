...
def _chroma_tem_documentos(db: Chroma) -> bool:
    """Retorna True se o ChromaDB possui ao menos 1 documento indexado."""
    try:
        col = db._collection
        return col.count() > 0
    except Exception:
...
def _extrair_normas_chroma(db: Chroma) -> set:
    """Extrai os códigos de norma APENAS do campo 'codigo' dos metadados do ChromaDB.
    Não escaneia o texto dos PDFs para evitar capturar normas referenciadas
    dentro do conteúdo mas que não estão cadastradas no sistema."""
    normas = set()
    try:
        col = db._collection
...
def _norma_esta_no_chroma(norma_ref: str | None, normas_chroma: set) -> bool:
    """Verifica se a norma referenciada (ex: 'NBR 5410') está presente no conjunto
    de normas extraídas do ChromaDB. Comparação flexível (ignora espaços extras)."""
