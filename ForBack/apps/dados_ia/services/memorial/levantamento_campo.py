...
def extrair_levantamento_campo_para_xlsx(dados_json: dict, ambiente_obj=None, template_path: str = None) -> bytes:
    ...
    if not is_form:
        if 'entidades' not in dados_json:
            log.info("Modo DXF — tentando extrair dict aninhado com 'entidades'")
    ...
    for i, amb in enumerate(ambientes[:MAX_AMBIENTES]):
        ...
        _preencher_amb_base(ws, r_amb, amb)
        ...
        if is_form:
            _preencher_eletrica(ws, r_ele, amb)
            _preencher_hidraulica(ws, r_hid, amb)
            _preencher_rede_spda(ws, r_red, amb)
            _preencher_incendio(ws, r_inc, amb)
            _preencher_cobertura(ws, r_cob, amb)
            _preencher_estruturas(ws, r_est, amb)
        else:
            escrever(ws, r_ele, 2, amb.get('nome'))
            escrever(ws, r_hid, 2, amb.get('nome'))
            escrever(ws, r_red, 2, amb.get('nome'))
            escrever(ws, r_inc, 2, amb.get('nome'))

        escrever(ws, r_est, 2, amb.get('nome'))
        escrever(ws, r_est, COL_EST_PILAR_C, dxf_est.get('pilares') or None)
        escrever(ws, r_est, COL_EST_VIGA_C, dxf_est.get('vigas') or None)
        escrever(ws, r_est, COL_EST_LAJE_C, dxf_est.get('lajes') or None)

        if not amb.get('tomadas'):
            escrever(ws, r_ele, COL_ELE_QUADROS, dxf_ele.get('quadros') or None)
            escrever(ws, r_ele, COL_ELE_CONDULE, dxf_ele.get('conduletes') or None)
            escrever(ws, r_ele, COL_ELE_TOMADAS, dxf_ele.get('tomadas') or None)
            escrever(ws, r_ele, COL_ELE_INTERR, dxf_ele.get('interruptores') or None)
            escrever(ws, r_ele, COL_ELE_LUM, dxf_ele.get('luminarias') or None)
            escrever(ws, r_ele, COL_ELE_DUTOS, dxf_ele.get('dutos_m') or None)
            escrever(ws, r_ele, COL_ELE_CABOS, dxf_ele.get('cabos_m') or None)
    ...
    escrever(ws, LINHA_LOG, 2, log_line)
