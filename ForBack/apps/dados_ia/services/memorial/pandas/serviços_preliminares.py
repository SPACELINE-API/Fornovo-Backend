...
colunas_servicos_preliminares6 = pd.MultiIndex.from_tuples(
    [
        ("Resíduo", "Comum V [m³]"),
        ("Resíduo", "Contaminado V [m³]"),
        ("", "Destino")
    ]
)


def servicos_preliminares(json_path, dxf_path):
    ...
    def _get_amb_sp(cx, cy, threshold=200):
        ...
        best = min(ambientes_cad_sp,
                   key=lambda a: math.hypot(cx - a["pos"][0], cy - a["pos"][1]))
        return best["nome"] if math.hypot(cx - best["pos"][0], cy - best["pos"][1]) <= threshold else "Não identificado"
    ...
    for _e in _alv_dem_lines:
        ...
        _alv_por_amb[_amb] = _alv_por_amb.get(_amb, 0.0) + _e["dados"].get("comprimento", 0.0)
    ...
    _fiacao_rem = {}
    for _t in _textos_auto:
        if ("fiação/duto enterrado a remover" in _t.get("conteudo", "").lower()
                and _t.get("posicao")):
            _cx, _cy = _t["posicao"][:2]
            _amb = _get_amb_sp(_cx, _cy)
    ...
    for _t in _txts_qa:
        ...
        if _id_pat.match(_cv):
            _pos = _t["posicao"]
            _cands = [
                _u for _u in _txts_qa
                if _num_pat.match(_u["conteudo"].strip())
                and _u.get("posicao")
                and abs(_u["posicao"][1] - _pos[1]) < 2.0
                and _u["posicao"][0] > _pos[0]
            ]
            _qtd = int(min(_cands, key=lambda _u: abs(_u["posicao"][0] - _pos[0]))["conteudo"])  if _cands else 1
            _esq_por_id[_cv] = _esq_por_id.get(_cv, 0) + _qtd

    _total_portas  = sum(_v for _k, _v in _esq_por_id.items() if _k.upper().startswith("P"))
    _total_janelas = sum(_v for _k, _v in _esq_por_id.items() if _k.upper().startswith("J"))
    ...
    for _e in _alv5_lines:
        ...
        _cy = (_e["dados"]["inicio"][1] + _e["dados"]["fim"][1]) / 2
        _amb = _get_amb_sp(_cx, _cy)
        _alv5_por_amb[_amb] = _alv5_por_amb.get(_amb, 0.0) + _e["dados"].get("comprimento", 0.0)

    def _comp_por_amb5(layer):
        _r = {}
        for _e in _entidades_auto:
            if _e.get("layer") != layer or _e.get("tipo") != "LINE" or "dados" not in _e:
                continue
            _cx = (_e["dados"]["inicio"][0] + _e["dados"]["fim"][0]) / 2
            _cy = (_e["dados"]["inicio"][1] + _e["dados"]["fim"][1]) / 2
    ...
    for _amb in sorted(_ambientes5):
        ...
        _rotulo_demolir = " / ".join(sorted(_demolir_txt_por_amb[_amb])) if _amb in _demolir_txt_por_amb else "a demolir"
        ...
        if _amb in _alv5_por_amb:
            ...
            df5.loc[idx_linha5, ("Alvenaria", "V [m³]")] = round(_comp_alv * _ESP_ALV * _H_ALV, 3)

        if _amb in _pilares5_amb:
            df5.loc[idx_linha5, ("Estrutura", "Pilar [m³]")]   = round(_pilares5_amb[_amb], 3)
        if _amb in _fundacoes5_amb:
            df5.loc[idx_linha5, ("Estrutura", "Fundação [m³]")] = round(_fundacoes5_amb[_amb], 3)
        if _amb in _vigas5_amb:
            df5.loc[idx_linha5, ("Estrutura", "Viga [m³]")]    = round(_vigas5_amb[_amb], 3)
        if _amb in _lajes5_amb:
            df5.loc[idx_linha5, ("Estrutura", "Laje [m³]")]    = round(_lajes5_amb[_amb], 3)
